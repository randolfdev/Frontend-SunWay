...
export default function ContaItem({ filterValue, showQuery }: ContaItemProps) {
  const Contas = [{ id: 1, Executor: "Usuário x", inicio: "10/04/2022", Registros: 7 }, { id: 2, Executor: "Usuário y", inicio: "10/26/2022", Registros: 3 }, { id: 3, Executor: "Usuário a", inicio: "11/04/2019", Registros: "10" }, { id: 4, Executor: "Usuário b", inicio: "11/27/2021", Registros: 43 }, { id: 5, Executor: "Usuário c", inicio: "10/11/2022", Registros: 20 }, { id: 6, Executor: "Usuário d", inicio: "12/11/2021", Registros: 14 }];
  let dateFilter: Date;
  if (filterValue > 0) {
      ...
      {Contas.map((val, key) => {
        ...
        if (dataInicio > dateFilter.getTime()) {
          if (val.Executor.toLowerCase().includes(textoPesquisa.toLowerCase())) {
            return (
              <div key={key} className="ContaItem shadow-lg">
                <div className="ContaItemHeader">
                  <div className="ContaItemId">{val.id}</div>
                  <span>Contas {key + 1}</span>
                </div>
                <div className="ContaCorpo">
